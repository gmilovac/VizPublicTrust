...
<head>
  ...
  <title>Trust in Government Agencies — UMAP Explorer without warmth and dominance</title>
...
<body>
  ...
  function renderPlot(data){
    ...
    const trace = {
      ...
      hovertemplate: data.map(d=> `<b>${d.Agency}</b><br>Trust: ${d.Trust.toFixed(2)}<br><i>Dimensions:</i><br>${TRUST_DIMS.map(dim=>`${dim}: ${d[dim].toFixed(2)}`).join('<br>')}<extra></extra>`),
      mode:'markers+text',
